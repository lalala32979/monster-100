...
for cell in sheet['A']:
    ...
    if cell.value is not None and cell.row > 4:
        ...
        print(cell.value , folder_name, folder_type)
        
        num_str = str(cell.value).zfill(3)
        
        word_file = '模板.docx'
        # 复制 Word 文件
        shutil.copy(word_file, f'./excel/{num_str}{folder_name}.docx')

        # 打开 Word 文件
        document = Document(f'excel/{num_str}{folder_name}.docx')
        ...
        # 保存 Word 文件
        document.save(f'excel/{num_str}{folder_name}.docx')
